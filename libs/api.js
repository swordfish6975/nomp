...
module.exports = function(logger, portalConfig, poolConfigs){
    ...
    const thread = fork('libs/buildUpdatedWebsite.js');
    thread.send({type:'init', portalConfig: JSON.stringify(portalConfig), poolConfigs: JSON.stringify(poolConfigs)});
    var buildUpdatedWebsite = function(){
            logger.special(logSystem, 'buildUpdatedWebsite', 'start');
            thread.send({type:'run'});
            logger.special(logSystem, 'buildUpdatedWebsite', 'stop');
    };

    thread.on('message', result => {
       var x = JSON.parse(result)
       portalStats.stats = x.stats;
       portalStats.statHistory = x.statHistory;
       portalStats.statPoolHistory = x.statPoolHistory;
	   statsRes = x.statsRes;
    });
    ...
    this.handleApiRequest = function(req, res, next){
        ...
        switch(req.params.method){
            case 'poolConfigs':
                ...
                res.end(JSON.stringify(poolConfigs));
                return;
            case 'portalConfig':
                res.header('Content-Type', 'application/json');
								...
								for(var pool in portalStats.stats.pools) {
								  for(var w in portalStats.stats.pools[pool].workers){
									  if (w.startsWith(address)) {
										...
										networkHashs = portalStats.stats.pools[pool].poolStats.networkHashs;
									  }
								  }
								}
								res.end(JSON.stringify({miner: address, totalHash: totalHash, totalShares: totalShares, networkHashs: networkHashs, immature: balances.totalImmature, balance: balances.totalHeld, paid: balances.totalPaid, workers: workers, history: history}));
							});
						});
					} else {
