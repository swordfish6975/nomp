...
module.exports = function(logger, portalConfig, poolConfigs){
	...
	this.getStatsString = function (cback) {
			cback(_this.statsString);
   };
   ...
   this.getBlocks = function (cback) {
	...
	function getWorkerStats(address) {
			...
			for (var h in statHistory) {
				for(var pool in statHistory[h].pools) {
					 if (address.length > 0 &&  address.startsWith(poolConfigs[pool].firstBit)) {
    ...
    this.getGlobalStats = function(callback){
        ...
        async.each(redisClients, function(client, callback){
                    ...
                    for(var i = 0; i < replies.length; i += commandsPerCoin){
                        ...
                        if (replies[i + 2]) {
                            if (replies[i + 2].coinmarketcap) {
                                ...
                                marketStats.display_price = marketStats.price_usd;
                                marketStats.display_unit = 'USD';

                            }
                            if (replies[i + 2].tradeogre) {
                                 marketStats = replies[i + 2] ? (JSON.parse(replies[i + 2].tradeogre)[0] || 0) : 0;
                                     logger.debug(logSystem, 'Global1', marketStats.price);
                                 var btc = replies[i + 2] ? (JSON.parse(replies[i + 2].bitcoinaverage)[0] || 0) : 0;
				 marketStats.display_price = replies[i + 2] ? (marketStats.price * parseInt(btc['BTCAUD']['last'])) : 0;
                                 marketStats.display_unit = 'AUD';
                            }
							if (replies[i + 2].myspeedtrade) {
                                 marketStats = replies[i + 2] ? (JSON.parse(replies[i + 2].myspeedtrade)[0] || 0) : 0;
                                     logger.debug(logSystem, 'Global2', marketStats.ticker.last);
                                 var btc = replies[i + 2] ? (JSON.parse(replies[i + 2].bitcoinaverage)[0] || 0) : 0;
				 marketStats.display_price = replies[i + 2] ? (marketStats.ticker.last * parseInt(btc['BTCAUD']['last'])) : 0;
                                 marketStats.display_unit = 'AUD';
                            }
                        }
                        var coinStats = {
                            ...
                            blockExplorer: poolConfigs[coinName].coin.blockExplorer,
                            ...
                            poolStats: {
								...
								networkHashs: replies[i + 2] ? (replies[i + 2].networkHashs || 0) : 0, 
								networkHashsString: getReadableNetworkHashRateString(replies[i + 2] ? (replies[i + 2].networkHashs || 0) : 0), 
            ...
            Object.keys(allCoinStats).forEach(function(coin){
				...
				//todo???
                var _blocktime = poolConfigs[coin].coin.blockTime;
				var _networkHashRate = parseFloat(coinStats.poolStats.networkHashs) * 1.2;
                ...
                portalStats.algos[algo].miners += Object.keys(coinStats.miners).length;
            ...
            _this.statsRes = _this.statsString;
	...
	if (hashrate < 1000000) {
		return (Math.round(hashrate / 1000)  ).toFixed(2)+' kh/s';
	}
